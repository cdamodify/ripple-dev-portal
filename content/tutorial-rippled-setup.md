...
The script updates the installed `rippled` package within an hour of each new release. The `RANDOM_DELAY` staggers the update and restart so multiple `rippled` servers do not update all at the same time.
